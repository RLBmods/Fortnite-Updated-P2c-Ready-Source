...
#include "../other/fonts/iconcpp.h"
#include "../other/fonts/icons.h"
...
struct toggle_checkbox_animation {
    float animation;
    float disabled_animation;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float w = GetWindowWidth() - 40; // keicia ilgi
    ...
    static std::map <ImGuiID, toggle_checkbox_animation> animate;
    ...
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 1.0f : 0.f, 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.disabled_animation = ImLerp(it_anim->second.animation, !*v ? 0.15f : 0.f, 1.0f * (1.0f - ImGui::GetIO().DeltaTime));

    RenderNavHighlight(total_bb, id);

    ImU32 col_bg;
    col_bg = ImGui::GetColorU32(ImVec4(0.555f, 0.0f, 0.0f, it_anim->second.animation)); // purple 

    //Background
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(71, 60, 68), false, 9.0f);

    //Active
    RenderFrame(frame_bb.Min, frame_bb.Max, col_bg, false, 9.0f);

    //Circle anim
    if (*v)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, ImColor(0.33f, 0.33f, 0.33f, it_anim->second.animation), 30);
    else
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, ImColor(1.0f, 1.0f, 1.0f, it_anim->second.disabled_animation), 30);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 58));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, 23), window->DC.CursorPos + ImVec2(size, 58));

    const ImVec2 arrow_size = icons->CalcTextSizeA(16.0f, FLT_MAX, 0.0f, "F");
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(34, 34, 36, 150), 3.0f);
    ...
    PushFont(icons); {
        window->DrawList->AddText(ImVec2(clickable.Max.x - 30, (clickable.Min.y + clickable.Max.y) / 2 - arrow_size.y / 2), ImColor(1.0f, 1.0f, 1.0f, 0.2f + it_anim->second.arrow_anim), "H");
    } PopFont();
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(34, 34, 36, 200)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = ImGui::CalcItemWidth() - 250;
    ...
    // Render grab	
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.GetCenter().y), ImVec2(frame_bb.Max.x - 10, frame_bb.GetCenter().y), ImColor(100, 100, 100));
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImColor(255, 255, 255));
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 6, ImColor(80, 80, 80));
    }
   // window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImGui::GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    ...
    ImGui::RenderTextClipped(ImVec2(frame_bb.Min.x - 120, frame_bb.Min.y + 1), ImVec2(frame_bb.Min.x + 50, frame_bb.Min.y + 21), value_buf, value_buf_end, NULL, ImVec2(0.5, 0.5));
    if (label_size.x > 0.0f)
        ImGui::RenderText(ImVec2(frame_bb.Min.x - 429, frame_bb.Min.y + 1), label);
    ImGui::NewLine();
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, float size)
{
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, float size)
{
